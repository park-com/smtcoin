...
int YEAR_BLOCKS = 60*24*365;
int LAST_POW_BLOCK=YEAR_BLOCKS*20;  // 20 years
...
const string strMessageMagic = "SmtCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 1)
    {
        ...
        else if(pindexBest->nHeight >= 1 && pindexBest->nHeight < YEAR_BLOCKS * 4)
    {
        nSubsidy = 20 * COIN;
    }
        else if(pindexBest->nHeight >= YEAR_BLOCKS * 4 && pindexBest->nHeight < YEAR_BLOCKS * 8)
    {
        nSubsidy = 10 * COIN;
    }
        else if(pindexBest->nHeight >= YEAR_BLOCKS * 8 && pindexBest->nHeight < YEAR_BLOCKS * 12)
    {
        nSubsidy = 5 * COIN;
    }
        else if(pindexBest->nHeight >= YEAR_BLOCKS * 12 && pindexBest->nHeight < YEAR_BLOCKS * 16)
    {
        nSubsidy = 2.50 * COIN;
    }
        else if(pindexBest->nHeight >= YEAR_BLOCKS * 16 && pindexBest->nHeight < YEAR_BLOCKS * 20)
    {
        nSubsidy = 1.25 * COIN;
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if(pindexBest->nHeight < YEAR_BLOCKS)	
	{
		nSubsidy = 200 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(pindexBest->nHeight < 2 * YEAR_BLOCKS)	
	{
		nSubsidy = 20 * nRewardCoinYear * nCoinAge / 365;
	}
	else if (pindexBest->nHeight < 3 * YEAR_BLOCKS)
	{
